...
with
    ...
    static member AsLinearExpression<'Measure> (Value d:Decision<'Measure>) =
        LinearExpression<'Measure>.Value (Domain.Decision.AsLinearExpression d)
...
and LinearExpression<[<Measure>] 'Measure> =
    ...
    with
        ...
        static member inline (+) (l, r) =
            let (Value lExpr) = ((^a) : (static member AsLinearExpression: ^a -> LinearExpression<'Measure>) l)
            let (Value rExpr) = ((^b) : (static member AsLinearExpression: ^b -> LinearExpression<'Measure>) r)
